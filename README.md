...
- **图表 PNG 导出**：每张图标题栏「存为 PNG」一键保存，顶部「导出全部图表 PNG」依次导出所有已渲染图（2x 像素比，文件名自动带股票名）
- **明细 CSV 导出**：分红除息明细、送股/拆股明细、多股对比表各自标题栏「导出 CSV」，Excel 可直接打开（含 BOM、币种列头随市场联动，如港股标“港元”）
- **整页报告导出**：顶部「导出报告 PDF（打印）」调起系统打印，可“另存为 PDF”；打印版自动切换浅色主题、隐藏操作 UI，并把 ECharts 图表换成高清图渲染，报告抬头含股票、区间、初始投入与生成时间
