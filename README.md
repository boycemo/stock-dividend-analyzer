...
| **持仓管理** | 多只股票持仓追踪:成本基础 / 实时市值 / 盈亏 / 累计分红 / 静态股息率,跨 A/HK/US 市场统一展示 |
...
- 📈 **静态股息率（主口径）+ 历史分位**:静态口径看年度分红能力,TTM / 自然年作为参考并列展示
- 📒 **持仓管理**:成本/市值/盈亏/累计分红/静态股息率,支持本地保存与 CSV 导出
...
- 📊 **实时行情与分红**:拉取最新价 + 分红数据,显示市值 / 累计落袋 / 当前息率(静态口径)
...
- **静态股息率跨市场统一**:复用 DRIP 分析器的 `fetchDividendsEm`(A 股)+ `fetchKlinesTx`(港 / 美),不重复造轮子
...
| **当前股息率（静态口径）** | 最近完整年度已实施每股分红合计 / 最新收盘价(主口径,与同花顺个股页一致);TTM(最近 365 天实际到账)与自然年口径作为参考并列展示 |
| **股息率历史分位（静态口径）** | 当前静态股息率在**已加载的全部历史**交易日中的排名占比。如 60% = 高于历史约 60% 的交易日。**分位越高 → 股息率越高 → 分红相对股价越划算**(注意方向与"股价分位"相反:股价分位越高越贵) |
| **同期股价涨幅** | 区间内期末收盘价相对期初收盘价的涨跌幅(未计入分红) |
| **持仓盈亏率** | (当前市值 − 成本基础) / 成本基础,未计税费与分红落袋 |
| **累计分红(持仓)** | 最近完整年度已实施每股分红 × 持股数;A/HK/US 分别按人民币 / 港元 / 美元展示 |
